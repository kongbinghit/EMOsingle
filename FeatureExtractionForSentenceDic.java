...
public class FeatureExtractionForSentenceDic {
		...
		try {
			...
			while ((tempString = reader.readLine()) != null) {
				...
				if (!"".equals(text)) {
					...
					for(int n=0;n<7;n++){
						featureresultmulan += (n + 8) + " "+dic[n]+", ";
					}
//					System.out.println(featureresultmulan);
					for (int n = 0; n < unigramnum.length; n++) {
						if (unigramnum[n] == 1) {
							featureresult += (n + 1) + ":1 ";
							featureresultarff += (n + 1) + " 1, ";
							featureresultmulan += (n + 15) + " 1, ";
						}
					}
...
//					if (!weiboemo1.equals("none")) {
						if (featureresult.trim().equals("")) {
							...
							writeruniqueID.write(uniqueID + " cet_" + eid
									+ "_P1_S" + sentenceid + "\r\n");
							writermulan.write("{0 " + map.get("happiness")
									+ ", 1 " + map.get("like") + ", 2 "
									+ map.get("anger") + ", 3 "
									+ map.get("sadness") + ", 4 "
									+ map.get("fear") + ", 5 "
